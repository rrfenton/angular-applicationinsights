...
export class StackFrame {
    ...
    toString() {
        let functionName = this.method || '{anonymous}';
        let args = '(' + (this.args || []).join(',') + ')';
        let fileName = this.fileName ? ('@' + this.fileName) : '';
        let lineNumber = Tools.isNumber(this.lineNumber) ? (':' + this.lineNumber) : '';
        let columnNumber = Tools.isNumber(this.columnNumber) ? (':' + this.columnNumber) : '';
